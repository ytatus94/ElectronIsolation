// This is used to make 1 dim projection plots for efficiency
// projection on X axis (pt axis) and Y axis (eta axis).
// Usage: root -l yt1DEffPlots.C"(\"TightLLH\",\"isolTight\")"
//
#include <TROOT.h>
#include <TFile.h>
#include <TString.h>
...
void yt1DEffPlots(TString menu = "TightLLH", TString iso = "isolTight")
{
    //gStyle->SetOptStat(0);
    //SetAtlasStyle();
    ...
    for (int pt_bin = 2; pt_bin < eff_data_central_value->GetXaxis()->GetNbins() + 1; pt_bin++) {
        ...
        make_1D_plot("#eta", "Efficiency", folder_name, TString(pt_range), pt_bin_low, pt_bin_up,
                     data_central_value, data_central_value_clone, mc_central_value, mc_central_value_clone);
    }

    //
    // projection on pt axis
    //
    for (int eta_bin = 2; eta_bin < eff_data_central_value->GetYaxis()->GetNbins() + 1; eta_bin++) {
        ...
        make_1D_plot("E_{T} [MeV]", "Efficiency", folder_name, TString(eta_range), eta_bin_low, eta_bin_up,
                     data_central_value, data_central_value_clone, mc_central_value, mc_central_value_clone);
    }
}
