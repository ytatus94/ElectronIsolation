...
TH2F* Make2DHistogram(TString key);
			...
			for (int i_temp = 0; i_temp < N_temp; i_temp++) {
				for (int i_tagID = 0; i_tagID < N_tagID; i_tagID++) {
					for (int i_mllWindow_x = 0; i_mllWindow_x < N_mllWindow_x; i_mllWindow_x++) {
    					...
    					TH2F *h_EffData_CentralValue_minDeltaR = Make2DHistogram("EffData_CentralValue_minDeltaR_"+ProbeIDAndVariationName);
						TH2F *h_EffData_StatError_minDeltaR    = Make2DHistogram("EffData_StatError_minDeltaR_"   +ProbeIDAndVariationName);
						TH2F *h_EffMC_CentralValue_minDeltaR   = Make2DHistogram("EffMC_CentralValue_minDeltaR_"  +ProbeIDAndVariationName);
						TH2F *h_EffMC_StatError_minDeltaR      = Make2DHistogram("EffMC_StatError_minDeltaR_"     +ProbeIDAndVariationName);
						TH2F *h_SF_CentralValue_minDeltaR      = Make2DHistogram("SF_CentralValue_minDeltaR_"     +ProbeIDAndVariationName);
						TH2F *h_SF_StatError_minDeltaR         = Make2DHistogram("SF_StatError_minDeltaR_"        +ProbeIDAndVariationName);
						...
						for(int etbin = 1; etbin <= N_et_bins ; ++etbin) {
							for(int minDeltaRbin = 1; minDeltaRbin <= N_dR_bins; ++minDeltaRbin) {
								...
								h_EffData_StatError_minDeltaR->   SetBinContent(etbin, minDeltaRbin, 0);
								...
								h_EffMC_StatError_minDeltaR->     SetBinContent(etbin, minDeltaRbin, 0);
								...
								h_SF_StatError_minDeltaR->        SetBinContent(etbin, minDeltaRbin, 0);
							}
						}
