...
void compareSFWithFilesInTools()
{
	TString menu = "TightLLH";
	TString iso = "isolGradientLoose";
	int pt = 4;
	...
	TString fileName1 = "/afs/cern.ch/user/y/yushen/afsWorkingArea/private/TagAndProbe/MyTagAndProbe_test29/20151218/MC15bd0z0_ScaleFactors.root";
...
//------------
	...
	TString fileName2 = "/afs/cern.ch/user/y/yushen/afsWorkingArea/private/TagAndProbe/MyTagAndProbe_test29/mc15bd0z0/output/ScaleFactors.root";
...
//------------
	...
	TString fileName3 = "/afs/cern.ch/user/y/yushen/test/data/isolation/efficiencySF.Isolation.TightLLH_d0z0_v8_isolGradientLoose.2015.13TeV.rel20p0.25ns.v04.root";

	TH2F *sf3      = get13TeV2DHistogram(fileName3, "0_9999999", "", "FullSim_sf");
	TH2F *sf3_tot  = get13TeV2DHistogram(fileName3, "0_9999999", "", "FullSim_tot");
	TH2F *sf3_stat = get13TeV2DHistogram(fileName3, "0_9999999", "", "FullSim_stat");
sf3->Draw("lego");
    TH1F* eta3[3][30]; // keeps total uncertainties
    TH1F* eta3_tmp[3][30]; // keeps statistical uncertainties
    TH1F* tot3_err[3][30];
    TH1F* sys3_err[3][30];
    TH1F* stat3_err[3][30];
    ...
    for (int pt = 0; pt < sf3->GetXaxis()->GetNbins() + 1; pt++) {
		...
		// SF
		eta3[2][pt]      = (TH1F *)sf3->ProjectionY("", pt, pt)->Clone();
		eta3_tmp[2][pt]  = (TH1F *)sf3->ProjectionY("", pt, pt)->Clone();
		tot3_err[2][pt]  = (TH1F *)sf3_tot->ProjectionY("", pt, pt)->Clone();
		stat3_err[2][pt] = (TH1F *)sf3_stat->ProjectionY("", pt, pt)->Clone();	
		fixHistogramError(eta3[2][pt],     tot3_err[2][pt]);	
		fixHistogramError(eta3_tmp[2][pt], stat3_err[2][pt]);	
	}
...
/*
	...
	// fileName3
	eta3[2][pt]->SetLineColor(kGreen+3);
	eta3[2][pt]->SetLineWidth(1);
	eta3[2][pt]->SetMarkerColor(kGreen+3);
	eta3[2][pt]->SetMarkerSize(1);
	eta3[2][pt]->SetMarkerStyle(20);
   	//eta3[2][pt]->Draw("e1,same");
   	eta3[2][pt]->Draw("e1");
	...
	leg->AddEntry(eta1[2][pt], "v8 (wrong file)", "lp");
	leg->AddEntry(eta2[2][pt], "v8 (correct file)", "lp");
	leg->AddEntry(eta3[2][pt], "ElectronEfficiencyCorrection", "lp");
	leg->SetBorderSize(0);
	leg->SetTextFont(42);
