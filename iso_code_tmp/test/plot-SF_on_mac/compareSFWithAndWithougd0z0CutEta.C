                                       ...
                                       int pt = 2)
{
    ...
    //double pad1_Y_max = 1.1;
    //double pad1_Y_min = 0.91;

	TString pad2_X_title = "#eta";
	TString pad2_Y_title = "Correct/Wrong";
...
//------------
    ...
    FindOptimalRange(low, high, eta1[2][pt], eta2[2][pt]);
    double pad1_Y_max = high;
    double pad1_Y_min = low;
	...
	leg->AddEntry(eta1[2][pt], "v8 (use the correct file)", "lp");
	leg->AddEntry(eta2[2][pt], "v8 (use the wrong file)", "lp");
	leg->SetBorderSize(0);
	leg->SetTextFont(42);
    ...
    TH1F *ratio = (TH1F *)eta1[2][pt]->Clone();
    ratio->Divide(eta2[2][pt]);
    
    high =-99;
    low = 99;
    FindOptimalRange(low, high, ratio);
    double pad2_Y_max = high;
    if(low<0) low =0;
    double pad2_Y_min = low;
	TH1F *frame_left;
	...
	c1->SaveAs("plots/comp_d0z0_MC15b_pT" + TString::Itoa(ptLowEdge,10) + "-" + TString::Itoa(ptUpEdge,10) + "_" + folder + ".pdf", "pdf");
}
