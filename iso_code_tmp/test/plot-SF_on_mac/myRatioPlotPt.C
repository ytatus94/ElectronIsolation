...
//--------------------
	...
	TString fileName = "~/Desktop/ScaleFactors.root";
...
//--------------------
    ...
    float high = 0, low = 0;
    FindOptimalRange(high, low, pt[0][eta], pt[1][eta]);
    pad1_Y_max = high;
    if (low < 0.) {
        pad1_Y_min = 0.;
    }
	...
	pt[0][eta]->GetXaxis()->SetRangeUser(7000, 150000);
   	pt[0][eta]->Draw("E0"); // E0: Draw error bars. Markers are drawn for bins with 0 contents.
    ...
    FindOptimalRange(high, low, pt[2][eta], pt_tmp[2][eta]);
    pad2_Y_max = high;
    if (low < 0.) {
        pad2_Y_min = 0.;
    }
	...
	pt[2][eta]->SetFillColor(kOrange-3);
   	pt[2][eta]->Draw("E2,SAME"); // E2: Draw error bars with rectangles.
